...
from task3_model1 import ImprovedModel, ImprovedTrainer
...
def create_plots(trainer: Trainer, trainer1: Trainer, name: str):
    ...
    utils.plot_loss(trainer.TRAIN_LOSS, label="Training loss (original)")
    utils.plot_loss(trainer.VALIDATION_LOSS, label="Validation loss (original)")
    utils.plot_loss(trainer.TEST_LOSS, label="Testing Loss (original)")
    utils.plot_loss(trainer1.TRAIN_LOSS, label="Training loss (model 1)")
    utils.plot_loss(trainer1.VALIDATION_LOSS, label="Validation loss (model 1)")
    utils.plot_loss(trainer1.TEST_LOSS, label="Testing Loss (model 1)")
    plt.legend()
    plt.subplot(1, 2, 2)
    plt.title("Accuracy")
    #utils.plot_loss(trainer.TRAIN_ACC, label="Training Accuracy")
    utils.plot_loss(trainer.VALIDATION_ACC, label="Validation Accuracy (original)")
    utils.plot_loss(trainer.TEST_ACC, label="Testing Accuracy (original)")
    utils.plot_loss(trainer1.VALIDATION_ACC, label="Validation Accuracy (model 1)")
    utils.plot_loss(trainer1.TEST_ACC, label="Testing Accuracy (model 1)")
    plt.legend()
...
if __name__ == "__main__":
    ...
    dataloaders1 = load_cifar10(batch_size)
    model1 = ImprovedModel(image_channels=3, num_classes=10)
    trainer1 = ImprovedTrainer(
        batch_size,
        learning_rate,
        early_stop_count,
        epochs,
    ...
    create_plots(trainer, trainer1, "task3c")
    print(
        ...
        f"Train Loss: {trainer.TRAIN_LOSS.popitem(last = True)}",
